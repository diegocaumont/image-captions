...
def load_models():
    ...
    # Set CLIP model to evaluation mode and move to CPU
    clip_model.eval()
...
def stream_chat(
    ...
    """
    # Clear CPU cache if necessary (not typically required)
    # torch.cuda.empty_cache()  # Removed for CPU
    ...
    try:
        ...
        return prompt_str, "Error processing image."
    ...
    with torch.no_grad():
        ...
        embedded_images = embedded_images.to("cpu")
    ...
    try:
        ...
        # If apply_chat_template is not available
        convo_string = ""
        for message in convo:
            convo_string += f"{message['role']}: {message['content']}\n"
        convo_string += "Assistant:"
    ...
    convo_tokens = convo_tokens.squeeze(0).to("cpu")   # Remove batch dimension
    prompt_tokens = prompt_tokens.squeeze(0).to("cpu")
    ...
    if len(eot_id_indices) >= 2:
        ...
        preamble_len = prompt_tokens.shape[0]
    ...
    input_ids = torch.cat([
        ...
        torch.zeros((1, embedded_images.shape[1]), dtype=torch.long),   # Dummy tokens
        convo_tokens[preamble_len:].unsqueeze(0),
    ], dim=1).to("cpu")
...
def main():
    ...
    # Log the number of CPU cores
    num_cores = multiprocessing.cpu_count()
    print(f"Number of CPU cores available: {num_cores}")
    
    clip_model, image_adapter, tokenizer, text_model = load_models()
